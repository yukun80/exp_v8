...
class Results(SimpleClass):
    ...
    Attributes:
        ...
        orig_shape (tuple): (高度，宽度)格式的原始图像形状。
        boxes (Boxes, optional): 包含检测边界框的对象。
        masks (Masks, optional): 包含检测掩码的对象。
        probs (Probs, optional): 包含分类任务的类概率的对象
        keypoints (Keypoints, optional): 包含每个对象的检测到的关键点的对象
        speed (dict): 预处理、推理和后处理速度字典(ms/image)。
        names (dict): 类名字典。
        path (str): 镜像文件的路径。
    ...
    def __init__(
        ...
        Args:
            ...
            boxes (torch.tensor, optional): 每次检测的边界框坐标的二维张量。
    ...
    def plot(
        ...
        """在输入的RGB图像上绘制检测结果。接受numpy数组(cv2)或PIL映像。
        Args:
        ...
        # 初始化了一个用于图像标注的 Annotator 对象，配置了多个选项以便根据不同的需要对图像进行标注。
        annotator = Annotator(
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                line = (
                    c,
                    *(d.xyxyxyxyn.view(-1) if is_obb else d.xywhn.view(-1)),
                )
                if masks:
                    seg = (
    ...
    def summary(self, normalize=False, decimals=5):
        ...
        for i, row in enumerate(
            ...
            box = (
                (row.xyxyxyxy if is_obb else row.xyxy)
                .squeeze()
                .reshape(-1, 2)
                .tolist()
            )
...
class Boxes(BaseTensor):
    """
    管理检测框，提供方便的访问和操作框坐标，置信度评分，类标识符，以及可选的跟踪id。支持多种框坐标格式，包括绝对和归一化形式。

    Attributes:
        data (torch.Tensor): 包含检测框及其相关数据的原始张量。
        orig_shape (tuple): 原始图像大小为元组(高度，宽度)，用于归一化。
        is_track (bool): 指示跟踪id是否包含在框数据中。
    ...
    def __init__(self, boxes, orig_shape) -> None:
        ...
        Args:
            boxes (torch.Tensor | numpy.ndarray): A tensor or numpy array containing the detection boxes, with
                shape (num_boxes, 6) or (num_boxes, 7). 最后两列包含置信度和类值。
...
class Masks(BaseTensor):
    ...
    def __init__(self, masks, orig_shape) -> None:
        """用给定的蒙版张量和原始图像形状初始化蒙版类。"""
        if masks.ndim == 2:
            masks = masks[None, :]
        super().__init__(masks, orig_shape)

    @property # 装饰器，可以直接读方法
    @lru_cache(maxsize=1) # 缓存装饰器，可以缓存方法的返回值。maxsize=1 指定了缓存的大小为 1，这意味着它只会缓存最近一次的结果。
    def xyn(self):
