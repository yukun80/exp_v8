    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
            self.mlp = RGBlock(
                in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer, drop=mlp_drop_rate
            )
        if self.mlp_branch:
            self.norm2 = norm_layer(hidden_dim)
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
...
class Base_VSSBlock(nn.Module):
    def __init__(
        self,
        in_channels: int = 0,  # new para
        hidden_dim: int = 0,
        # n: int = 1,
        drop_path: float = 0,
        norm_layer: Callable[..., torch.nn.Module] = partial(LayerNorm2d, eps=1e-6),
        ...
        ssm_d_state: int = 16,
        ssm_ratio=2.0,
        ssm_rank_ratio=2.0,  # new para
        ssm_dt_rank: Any = "auto",
        ssm_act_layer=nn.SiLU,
        ssm_conv: int = 3,
        ssm_conv_bias=True,
        ssm_drop_rate: float = 0,
        ssm_init="v0",  # old para
        forward_type="v2",
        # =============================
        mlp_ratio=4.0,
        mlp_act_layer=nn.GELU,
        ...
        **kwargs,
        ...
        self.op = SS2D(
            ...
            d_state=ssm_d_state,
            ssm_ratio=ssm_ratio,
            ssm_rank_ratio=ssm_rank_ratio,  # new para
            dt_rank=ssm_dt_rank,
            act_layer=ssm_act_layer,
            # ==========================
            d_conv=ssm_conv,
...
class VisionClueMerge(nn.Module):
    def __init__(self, dim, out_dim):
        ...
        self.pw_linear = nn.Sequential(
            nn.Conv2d(self.hidden, out_dim, kernel_size=1, stride=1, padding=0), nn.BatchNorm2d(out_dim), nn.SiLU()
        )
...
if __name__ == "__main__":
    # python -m ultralytics.nn.FYK_modules.mamba_yolo
    block1 = VSSBlock(128, 128).cuda()
    block2 = XSSBlock(128, 128).cuda()
    block3 = Base_VSSBlock(128, 128).cuda()
    convblock = VisionClueMerge(128, 256).cuda()
    input = torch.rand(3, 128, 64, 64).cuda()
    # output = block3(input)
    output = convblock(input)
