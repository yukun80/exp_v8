...
class Compose:
    ...
    def __setitem__(
        self, index: Union[list, int], value: Union[list, int]
    ) -> None:
        """Retrieve a specific transform or a set of transforms using indexing."""
        assert isinstance(
            index, (int, list)
...
class Mosaic(BaseMixTransform):
    """
    Mosaic augmentation.
    实现了拼图增强技术，通过将多个（4 或 9）图像组合成一个单一的拼图图像来进行数据增强。
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4):
        ...
        assert (
            0 <= p <= 1.0
        ), f"The probability should be in range [0, 1], but got {p}."
...
class MixUp(BaseMixTransform):
    ...
    通过将两张图像及其标签按照一定比例混合在一起，以增强数据集的多样性。以下是对该类的分析和解释："""
...
class RandomPerspective:
    ...
    def apply_bboxes(self, bboxes, M):
        ...
        return (
            np.concatenate(
                (x.min(1), y.min(1), x.max(1), y.max(1)), dtype=bboxes.dtype
            )
            .reshape(4, n)
            .T
        )
    ...
    def __call__(self, labels):
        ...
        self.size = (
            img.shape[1] + border[1] * 2,
            img.shape[0] + border[0] * 2,
        )  # w, h
        # M is affine matrix
    ...
    def box_candidates(
        ...
        """
        基于一组阈值计算候选框。这种方法比较了盒子的特征
        在增强前后决定一个盒子是否为进一步处理的候选。
        ...
        Args:
            ...
            wh_thr (float, optional): 宽度和高度阈值(以像素为单位)。默认值是2。
            ar_thr (float, optional): 宽高比阈值。默认值是100。
            area_thr (float, optional): 面积比率阈值。默认值是0.1。
            eps (float, optional): 一个小的值来防止被0整除。默认值是1e-16。
...
class RandomHSV:
    """
    这个类负责对图像的色调，饱和度和值(HSV)通道进行随机调整。
    这些调整是随机的，但在一次又一次设定的范围内。
    """
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        # Scale ratio (new / old)
        # 计算了新图像尺寸（new_shape）与原图像尺寸（shape）在宽度和高度上的比例，然后取这两个比例中的最小值赋值给 r。
        # 这样做的目的是为了保持图像的长宽比，避免图像在缩放过程中产生扭曲。
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            self.contains_spatial = any(
                transform.__class__.__name__ in spatial_transforms
                for transform in T
            )
            self.transform = (
                A.Compose(
    ...
    def __call__(self, labels):
        ...
        if self.contains_spatial:
            ...
            labels["img"] = self.transform(image=labels["img"])[
                "image"
            ]  # transformed
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            if nl:
                masks, instances, cls = self._format_segments(
                    instances, cls, w, h
                )
    ...
    def __call__(self, labels: dict) -> dict:
        ...
        for label in sampled_labels:
            ...
            prompt = self.prompt_format.format(
                prompts[random.randrange(len(prompts))]
            )
            texts.append(prompt)
    ...
    pre_transform = Compose(
        [
            ...
            RandomPerspective(
                ...
                pre_transform=(
                    None if stretch else LetterBox(new_shape=(imgsz, imgsz))
                ),
            ),
        ]
    )
...
def classify_augmentations(
    ...
    if not disable_color_jitter:
        secondary_tfl += [
            T.ColorJitter(
                brightness=hsv_v, contrast=hsv_v, saturation=hsv_s, hue=hsv_h
            )
        ]

    final_tfl = [
        T.ToTensor(),
        # 233333 去掉归一化，因为已经手动归一化过了
...
# NOTE: keep this class for backward compatibility 233333
# class ToTensor:
